...
class Tags extends VuexModule implements TagState {
  ...
  changeLegendName(name: string, idx: number) {
    if(idx > -1){
      ...
      if(leg !== name) this.context.commit('editLegendName', {name: name, idx: idx});
    }
  }
  ...
  addLegendParameter(leg: any){
    ...
    if(legend !== undefined){
      ...
      // @ts-ignore
      this.context.commit('addParameterToLegend', {idx: idx, para: parameter});
    }else{
      const parameters = [];
      parameters.push(leg.parameter);
      let ls: LegendList = {} as LegendList;
      ls.name = leg.lname;
