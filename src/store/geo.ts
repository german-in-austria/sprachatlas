...
async function init () {
  ...
  const sfb = await (await fetch('geojson/SFB_Dialektregionen.geojson')).json() as geojson.FeatureCollection;
  const vlbg = await (await fetch('geojson/vbg.geojson')).json() as geojson.FeatureCollection;
  geoStore.dialektregionen = concatGeoJSON(sfb, vlbg) as geojson.FeatureCollection;
}
